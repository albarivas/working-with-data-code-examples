...
import apexCreateMixedRecords from "c/apexCreateMixedRecords";
import { ShowToastEventName } from "lightning/platformShowToastEvent";
import { reduceErrors } from "c/ldsUtils";
import createContactAndOpportunity from "@salesforce/apex/apexCreateMixedRecordsControllerLwc.createContactAndOpportunity";

// Mocking imperative Apex method call
jest.mock(
  "@salesforce/apex/apexCreateMixedRecordsControllerLwc.createContactAndOpportunity",
  () => {
    return {
      default: jest.fn()
    };
...
describe("c-create-mixed-records-apex", () => {
  ...
  it("shows error toast message when there is an error", async () => {
    ...
    expect(handler.mock.calls[0][0].detail.message).toBe(
      "Error creating records: " + reduceErrors(APEX_OPERATION_ERROR).join(", ")
    );
  });
