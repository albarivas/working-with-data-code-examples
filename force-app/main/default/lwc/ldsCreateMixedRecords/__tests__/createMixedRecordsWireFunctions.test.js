...
import ldsCreateMixedRecords from "c/ldsCreateMixedRecords";
...
describe("c-create-mixed-records-wire-functions", () => {
  ...
  it("passes the user input to the createRecord LDS function correctly", async () => {
    ...
    expect(createRecord.mock.calls[1]).toEqual(CREATE_OPPORTUNITY_PARAMETERS);
  });
  ...
  it("shows error toast message when there is an error", async () => {
    ...
    expect(handler.mock.calls[0][0].detail.message).toBe(
      "Error creating records: " + reduceErrors(LDS_OPERATION_ERROR).join(", ")
    );
